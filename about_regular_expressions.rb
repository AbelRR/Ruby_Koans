...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    # * would fail when string to query is empty
  end
  ...
  #
  # Why?
  # these greedy operations are finding the first values they find.
  ...
  # ## Alternations offer the ability to identify different char patterns if first ones fail
  # ------------------------------------------------------------------
  ...
  def test_gsub_is_like_find_and_replace_all # find all occurrences of the pattern
    assert_equal "one LOLLOLLOL", "one two-three".gsub(/(t\w*|-)/,'LOL') { $1[0, 1] }
  end
end
